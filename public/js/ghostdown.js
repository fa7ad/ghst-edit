/* eslint-disable no-redeclare, no-empty, no-unused-vars, no-constant-condition */
/* globals CodeMirror, define, jQuery */

function hasOwnProperty(prop, obj) {
  return Object.prototype.hasOwnProperty.call(obj, prop);
}

function propertyIsEnumerable(prop, obj) {
...
window.CodeMirror = (function () {
  ...
  function CodeMirror(place, options) {
    ...
    for (var opt in defaults)
      if (!hasOwnProperty(opt, options) && hasOwnProperty(opt, defaults)) options[opt] = defaults[opt];
    ...
    operation(this, function () {
      for (var opt in optionHandlers)
        if (propertyIsEnumerable(opt, optionHandlers)) optionHandlers[opt](this, options[opt], Init);
      ...
      if (markers)
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          var id = cm.options.gutters[k],
            found = hasOwnProperty(id, markers) && markers[id];
          if (found)
  ...
  function restartBlink(cm) {
    ...
    display.blinker = setInterval(function () {
      on = !on;
      display.cursor.style.visibility = display.otherCursor.style.visibility = on ? '' : 'hidden';
  ...
  CodeMirror.prototype = {
    ...
    setOption: function (option, value) {
      ...
      if (hasOwnProperty(option, optionHandlers)) operation(this, optionHandlers[option])(this, value, old);
    },
    ...
    getHelper: function (pos, type) {
      if (!hasOwnProperty(type, helpers)) return;
  ...
  CodeMirror.resolveMode = function (spec) {
    if (typeof spec == 'string' && hasOwnProperty(spec, mimeModes)) {
      spec = mimeModes[spec];
    } else if (spec && typeof spec.name == 'string' && hasOwnProperty(spec.name, mimeModes)) {
  ...
  CodeMirror.getMode = function (options, spec) {
    ...
    if (hasOwnProperty(spec.name, modeExtensions)) {
      var exts = modeExtensions[spec.name];
      for (var prop in exts) {
        if (!hasOwnProperty(prop, exts)) continue;
        if (hasOwnProperty(prop, modeObj)) modeObj['_' + prop] = modeObj[prop];
  ...
  CodeMirror.extendMode = function (mode, properties) {
    var exts = hasOwnProperty(mode, modeExtensions) ? modeExtensions[mode] : (modeExtensions[mode] = {});
  ...
  CodeMirror.registerHelper = function (type, name, value) {
    if (!hasOwnProperty(type, helpers)) helpers[type] = CodeMirror[type] = {};
  ...
  var LineWidget = (CodeMirror.LineWidget = function (cm, node, options) {
    if (options) for (var opt in options) if (hasOwnProperty(opt, options)) this[opt] = options[opt];
    this.cm = cm;
    this.node = node;
  });
  ...
  for (var prop in Doc.prototype)
    if (hasOwnProperty(prop, Doc.prototype) && indexOf(dontDelegate, prop) < 0)
  ...
  function copyObj(obj, target) {
    if (!target) target = {};
    for (var prop in obj) if (hasOwnProperty(prop, obj)) target[prop] = obj[prop];
    return target;
  }
  ...
  function isEmpty(obj) {
    for (var n in obj) if (hasOwnProperty(n, obj) && obj[n]) return false;
    return true;
  }
  ...
  // various browsers.
  var spanAffectsWrapping = function spanAffectsWrapping() {
    return false;
  };
  if (gecko)
    // Only for "$'"
...
CodeMirror.defineMode(
  ...
  function (cmCfg, modeCfg) {
    var htmlFound = hasOwnProperty('text/html', CodeMirror.mimeModes);
    ...
    var getMode = (function () {
      ...
      var list = [];
      for (var m in CodeMirror.modes) if (propertyIsEnumerable(m, CodeMirror.modes)) list.push(m);
...
(function ($, ShowDown, CodeMirror) {
  ...
  $(function () {
    ...
    function updateImagePlaceholders(content) {
      var imgPlaceholders = $(document.getElementsByClassName('rendered-markdown')[0])
        .find('p')
        .filter(function () {
          return /^(?:\{<(.*?)>\})?!(?:\[([^\n\]]*)\])(?:\(([^\n\]]*)\))?$/gim.test($(this).text());
        });
            ...
            .dropzone({
              ...
              success: function (file, response) {
                var holderP = $(file.previewElement).closest('p'),
                  ...
                  imgHolderMardown = $('.CodeMirror-code')
                    .find('pre')
                    .filter(function () {
                      return (
                        /^(?:\{<(.*?)>\})?!(?:\[([^\n\]]*)\])(?:\(([^\n\]]*)\))?$/gim.test(self.text()) &&
                        self.find('span').length === 0
                      );
                    }),
                  // Get markdown
